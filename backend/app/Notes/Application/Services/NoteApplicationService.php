...
class NoteApplicationService
{
    ...
    public function deleteNote(int $id, int $userId): ?Note
    {
        ...
        if ($result) {
            event(new NoteDeletedEvent($note->id, $userId, $note->connector_id));
        }
    ...
    public function unshareNote(int $id, int $userId): ?Note
    {
        $note = $this->noteRepository->getUserNote($id, $userId);
        ...
        $note->changeSharingType(NoteSharingType::PRIVATE, null);
     ...
     */
    public function getDeltaNotesByUser(int $userId, string $deltaString, ?int $limit = null): array
    {
        $delta = new DateTime($deltaString);

        return $this->noteRepository->findDeltaByUser($userId, $delta, $limit);
    }
}
