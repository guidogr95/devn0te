...
export function useShareNoteDialog({ note, onUnshare }: Props) {
  ...
  const linkRef = useRef<HTMLInputElement>(null);
	...
	const { handleShareNote, handleUnshareNote } = useNotesActions();
  ...
  const copyLink = () => {
	...
	return {
		...
		copied,
		isLoadingShareNote,
		onUnshare: onUnshare ?? (() => handleUnshareNote(note.id))
	};
}
