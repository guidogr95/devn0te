...
export const setUpdateNoteAbortController = (state: NotesState, action: PayloadAction<{ id: number, controller: AbortController | null }>) => {
  state.updateNoteAbortControllerMap[action.payload.id] = action.payload.controller;
};

export const cancelUpdateNoteAbortController = (_state: NotesState, _action: PayloadAction<{ id: number }>) => {
};
